...
describe('microsite', () => {
  ...
  describe('on xhr requests', () => {
    ...
    describe('with successful form submission', () => {
      it('should set success headers', done => {
        res_spy = sinon.spy(res_mock, 'writeHead')
        locals = {
          status: 200,
    ...
    describe('with unsuccessful form submission', () => {
  ...
  describe('on non-xhr requests', () => {
    ...
    it('should include required fields', done => {
      var result
      res_mock.end = function (html) { result = html }
      microsite(req_mock, res_mock)
      var browser = new Browser()
      browser.load(result, () => {
        required_fields.forEach(field => {
          browser.assert
            .elements(`form#contact-form [name="${field}"][required]`)
        })
        done()
      })
    })
